...
import json
import os
from main import load_config, setup_logging, get_logger, get_all_agents, coordinator, ProposalGenerator
...
async def process_rfp_pdf(pdf_path: str, client_name: str, project_name: str,
                         project_type: str, rfp_config: dict, config: dict, logger):
    """Process RFP PDF and create proposal request"""
    try:
        # Initialize RFP extractor
        extractor = RFPExtractor(config)

        # Extract content from PDF
        rfp_content = await extractor.extract_from_pdf(pdf_path)
        ...
        logger.info(f"✓ RFP processed successfully: {len(rfp_content)} characters extracted")
        return request

    except Exception as e:
        logger.error(f"❌ Failed to process RFP PDF: {e}")
        raise
